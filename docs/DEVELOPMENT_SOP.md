# KTW Bot 開發標準作業程序 (Development SOP)

> 本文件整合了專案的核心價值觀與具體執行規範，為開發工作的最高指導原則。

---

> [!CAUTION] > **⚠️ 強制性提醒 (MANDATORY)**
>
> 在進行任何**新增、維護、修改**工作之前，必須：
>
> 1. ✅ **先完整閱讀本文件** (DEVELOPMENT_SOP.md)
> 2. ✅ **確認相關規範與原則**
> 3. ✅ **再開始規劃與實作**
>
> **違反此原則的變更將被視為不符合專案標準。**

---

## 0. 專案定位與發展策略

> **策略方向**：以 SaaS 化為最終目標，逐步淘汰單一客戶專用系統，走向通用型全系統 SaaS 架構。

### 專案定位表

| 專案                  | 類型          | 定位                                | 狀態                      |
| :-------------------- | :------------ | :---------------------------------- | :------------------------ |
| **ktw-saas-platform** | SaaS 管理平台 | Payload CMS + 多租戶管理            | 🚀 **主力開發**           |
| **ktw-saas-bot**      | SaaS LINE Bot | 通用型多租戶 Bot                    | 🚀 **主力開發**           |
| **ktw-bot**           | 舊版 Bot      | KTW Hotel 專用 (LINEBOT, Admin-Web) | ⏸️ **維護模式**，逐步淘汰 |
| **ktw-web-pms**       | 行業專用系統  | 旅宿觀光業 Web-PMS (SaaS 化為終旨)  | 📋 **待商確**             |
| **ktw-docs**          | 文檔中心      | 通用開發文檔                        | ✅ 持續維護               |
| **ktw-oracle-to-pg**  | 遷移工具      | Oracle → PostgreSQL                 | ⏸️ 視需求維護             |

### 發展路線圖

```
現況                        中期目標                     最終目標
┌─────────────────┐        ┌─────────────────┐        ┌─────────────────┐
│   ktw-bot       │   →    │ ktw-saas-bot    │   →    │   通用型 SaaS   │
│   (單一客戶)     │        │ (多租戶 MVP)     │        │   Bot 平台      │
└─────────────────┘        └─────────────────┘        └─────────────────┘
         ↓                          ↓                          ↓
  KTW Hotel 專用           KTW + 2-3 測試客戶           N 個飯店客戶
```

### 開發優先順序

1. **優先**：`ktw-saas-platform` + `ktw-saas-bot` (SaaS 核心)
2. **次要**：`ktw-bot` 維護 (僅修復重大 Bug，不新增功能)
3. **待定**：`ktw-web-pms` (旅宿觀光業專用方案)

---

## 1. 核心理念與原則 (Core Principles)

### 1.1 核心價值

- **專業紀錄**：專業並記錄筆記結果，同步上傳 Notion/GitHub。都需採繁體中文，如需用到英文需加上中文註解 (Chinese Annotation)，尊崇 DRY (Don't Repeat Yourself) 原則。
  - **全語境繁體中文**：從 AI 的內部思考 (`thought`)、工具調用參數 (`TaskName`, `TaskStatus`, `Summary`) 到最終輸出，**全面禁止使用英文或簡體中文**（除非是程式碼關鍵字、API 名稱或專有名詞）。**若因技術必要需使用英文詞彙，必須在後方括號補充繁體中文註解 (Chinese Annotation)**。
- **簡潔至上**：恪守 KISS (Keep It Simple, Stupid) 原則，崇尚簡潔與可維護性，避免過度工程化與不必要的防禦性設計。
- **深度分析**：立足於第一性原理 (First Principles Thinking) 剖析問題，並善用工具以提升效率。
- **事實為本**：以事實為最高準則。若有任何誤解，懇請坦率扶正，助我精進。
- **研究導向** ⭐：任何資訊提供前必須：
  1. ✅ **上網查詢**：搜尋相關領域的專業知識與慣例
  2. ✅ **深度思考**：結合上下文、實際數據、業務邏輯綜合分析
  3. ✅ **全面判斷**：考慮所有可能的解釋與方案
  4. ✅ **詳細內容**：提供完整資訊，不遺漏重要細節
  5. ✅ **利弊參考**：若有多種可能解釋或方案，列出各自利弊供決策

### 1.2 開發與流程

- **開發工作流程**：均採 **System + Function Calling** 的最佳自動化客服人員結構。
- **漸進式開發**：通過多輪對話迭代，明確並實現需求。在著手任何設計或編碼工作前，必須完成前期調研並釐清所有疑點，需參考先前資料或筆記。
- **結構化流程**：嚴格遵循「構思方案 → 提煉骨架 → 分解為具體任務」的作業順序。
- **版本確認規則** ⭐：凡建構**核心技術元件**或**部署環境**時，必須：
  1. ✅ 先上網查詢該技術/框架的**最新穩定版本** (LTS / Stable)
  2. ✅ 與使用者確認版本後再施做
  - **適用範圍**：Node.js、Python、Docker 基底映像、資料庫版本、主要框架版本 (Next.js, Flask, Payload CMS 等)
  - **禁止事項**：❌ 直接使用本地環境版本或憑印象指定版本

### 1.3 AI + MCP + Skills 架構原則 ⭐

> **核心理念**：採用「AI 核心 + MCP Server + Skills 模組」三層架構，讓 AI 透過標準化協議連接外部工具與數據源，並透過模組化的 Skills 封裝業務邏輯。

**整體架構**：

```
用戶訊息
    ↓
┌─────────────────────────────────────────────────────────┐
│  AI Core (Gemini / GPT)                                 │
│  • System Prompt                                        │
│  • Function Calling / Tool Use                          │
│  • 意圖識別與回覆生成                                     │
└─────────────────────────────────────────────────────────┘
    ↓                           ↓
┌─────────────────────┐   ┌─────────────────────┐
│  MCP Servers        │   │  Skills (模組化技能)  │
│  (外部數據源)        │   │  (業務邏輯封裝)       │
├─────────────────────┤   ├─────────────────────┤
│ • PMS API           │   │ • 當日訂房 Skill    │
│ • Weather API       │   │ • 訂單查詢 Skill    │
│ • Database          │   │ • VIP 服務 Skill    │
│ • 房控系統           │   │ • 內部查詢 Skill    │
└─────────────────────┘   └─────────────────────┘
    ↓                           ↓
    └───────────── 結果合併 ─────────────┘
                    ↓
              AI 生成自然回覆
```

**三層架構說明**：

| 層級   | 名稱       | 說明                          | 範例                     |
| :----- | :--------- | :---------------------------- | :----------------------- |
| **L1** | AI Core    | 意圖識別、對話管理、回覆生成  | Gemini + System Prompt   |
| **L2** | MCP Server | 外部 API/資料庫的標準化連接器 | PMS MCP、Weather MCP     |
| **L3** | Skills     | 可重用的業務邏輯模組          | `same_day_booking.skill` |

**MCP Server 規範**：

- **定義**：Model Context Protocol Server，提供 AI 存取外部資源的標準介面
- **優點**：解耦 AI 與外部系統，易於擴充與替換
- **實作方式**：每個 MCP Server 獨立部署，透過標準 JSON-RPC 通訊

**Skills 模組化規範**：

- **定義**：封裝特定業務流程的可重用模組
- **結構**：每個 Skill 必須包含 `SKILL.md` 說明文件
- **核心原則**：
  - ✅ **單一職責**：一個 Skill 只處理一種業務流程
  - ✅ **自包含**：Skill 內含所需的狀態機、驗證邏輯、回覆模板
  - ✅ **可組合**：Skill 可以呼叫其他 Skill 或 MCP Server
  - ✅ **可測試**：Skill 可以獨立測試，不依賴其他模組

**Skill 目錄結構規範**：

```
skills/
├── same_day_booking/           # 當日訂房 Skill
│   ├── SKILL.md                # 必須：Skill 說明文件
│   ├── __init__.py
│   ├── machine.py              # 狀態機邏輯
│   ├── validators.py           # 驗證器 (電話、日期等)
│   ├── templates.py            # 回覆模板
│   └── tests/                  # 單元測試
│       └── test_machine.py
├── order_query/                # 訂單查詢 Skill
│   ├── SKILL.md
│   └── ...
└── vip_service/                # VIP 服務 Skill
    ├── SKILL.md
    └── ...
```

**SKILL.md 必要內容**：

```markdown
# [Skill 名稱]

## 功能描述

一句話說明此 Skill 的用途

## 觸發條件

- 關鍵字：...
- 意圖：...

## 狀態流程

idle → collecting_info → confirming → completed

## 輸入參數

| 參數  | 類型 | 必填 | 說明     |
| :---- | :--- | :--- | :------- |
| phone | str  | ✅   | 聯絡電話 |

## 輸出格式

成功/失敗的回覆模板

## 依賴

- MCP Server: PMS API
- 其他 Skill: 無
```

**模組化開發流程**：

1. **建立目錄**：在 `skills/` 下建立新的 Skill 目錄
2. **撰寫 SKILL.md**：定義功能、狀態流程、輸入輸出
3. **實作狀態機**：在 `machine.py` 中實作核心邏輯
4. **撰寫測試**：在 `tests/` 中撰寫單元測試
5. **註冊至主路由**：在 `bot.py` 中註冊新 Skill

**持續優化方式**：

1. **新增 Skill**：封裝新業務流程為獨立模組
2. **新增 MCP Server**：連接新的外部數據源
3. **調整 System Prompt**：優化 AI 判斷邏輯
4. **累積知識庫**：豐富 FAQ 與業務知識
5. **分析對話日誌**：找出 AI 誤判案例並修正

**禁止事項**：

- ❌ 不使用**脫離 AI 理解**的純硬編碼狀態機（應該先透過 AI 判定意圖，再由狀態機接手流程）
- ❌ 不使用硬編碼正則表達式取代 AI 理解（僅能在 AI 失敗時作為輔助校驗）
- ❌ 不繞過 AI 指令直接強行跳轉模組（除非是系統級重置）
- ❌ 不將 AI 邏輯與外部 API 直接耦合（必須透過 MCP Server 隔離）

### 1.4 對話系統架構原則：模組化狀態機 (Modular FSM) ⭐

> **適用範圍**：本原則專用於 `LINEBOT` (bot.py) 等需要處理複雜、有狀態之對話流程的組件。
>
> **核心理念**：結合「確定性的狀態機」以保證商務邏輯安全，並輔以「靈活的 AI 工具調用」提升服務溫度。

**主要組件**：

1.  **狀態機導向 (State-Machine Driven)**：使用 `ConversationStateMachine` 進行 Session 管理。這不是為了取代 AI，而是為了在 AI 判定意圖後，**強制確保商務流程的完整性**（如訂單確認必須經過電話校驗）。
2.  **處理器委派模式 (Handler Delegation Pattern)**：`bot.py` 主路由僅負責意圖分發。各功能塊（如 `OrderQueryHandler`, `VIPServiceHandler`）各自獨立管理其內部的任務邏輯。
3.  **工具調用模型 (Tool-Calling)**：這是 AI 的執行手腳。當狀態處於閒置 (`idle`) 時，由 AI 根據客人的自然語言自主決定調用哪些 tools（如 `update_guest_info`）。
4.  **MCP 整合**：外部 API 呼叫透過 MCP Server 進行，確保 AI 與外部系統解耦。

**架構優勢**：

- **安全與穩健**：狀態機保證了關鍵欄位不遺漏、不幻覺。
- **靈活性**：非關鍵流程由 AI 代理處理，提升對話的自然度與應變能力。
- **可擴展性**：新增功能只需建立新的 Skill 並註冊至路由，不會汙染現有邏輯。
- **持久化支援**：結合 SQLite 實現長效 Session，中斷後可無縫銜接。
- **標準化連接**：透過 MCP 協議連接外部系統，易於替換與維護。

### 1.5 SaaS 多租戶架構 ⭐

> **適用範圍**：ktw-saas-platform (Payload CMS)、ktw-saas-bot (LINE Bot) 的多租戶 SaaS 開發。
>
> **核心理念**：以 Payload CMS 為 SaaS 核心平台，透過租戶隔離機制，讓多個飯店客戶共用同一套系統。

**SaaS 整體架構**：

```
                        ┌─────────────────────────────┐
                        │      Zeabur Cloud (日本)     │
                        └─────────────────────────────┘
                                      │
        ┌─────────────────────────────┼─────────────────────────────┐
        ↓                             ↓                             ↓
┌───────────────────┐   ┌───────────────────┐   ┌───────────────────┐
│  Payload CMS      │   │  SaaS Bot         │   │  PostgreSQL       │
│  (管理平台)        │   │  (多租戶 Bot)      │   │  (共享資料庫)      │
├───────────────────┤   ├───────────────────┤   ├───────────────────┤
│ • 租戶管理        │   │ • 租戶路由        │   │ • RLS 隔離        │
│ • 用戶認證        │   │ • Skills 調用     │   │ • Schema 隔離     │
│ • 配置管理        │   │ • AI 對話處理     │   │ • 對話日誌        │
│ • Usage 追蹤      │   │ • MCP 整合        │   │ • 用量統計        │
└───────────────────┘   └───────────────────┘   └───────────────────┘
        │                       │
        │     Webhook URL       │
        │   /webhook/:tenant_id │
        ↓                       ↓
┌───────────────────────────────────────────────────────────────────┐
│                        租戶 (飯店客戶)                             │
├───────────────┬───────────────┬───────────────┬───────────────────┤
│   KTW Hotel   │   Hotel A     │   Hotel B     │   Hotel C ...     │
│   tenant_001  │   tenant_002  │   tenant_003  │   tenant_004      │
└───────────────┴───────────────┴───────────────┴───────────────────┘
```

**SaaS 分層架構**：

| 層級         | 組件             | 部署位置          | 隔離策略           |
| :----------- | :--------------- | :---------------- | :----------------- |
| **平台層**   | Payload CMS      | Zeabur (永久)     | 共享，用戶權限隔離 |
| **服務層**   | SaaS Bot         | Zeabur → 自架主機 | 租戶路由隔離       |
| **數據層**   | PostgreSQL (CMS) | Zeabur            | 共享               |
| **數據層**   | PostgreSQL (Bot) | 自架主機          | RLS / Schema 隔離  |
| **外部整合** | PMS MCP Server   | 各飯店本地        | 完全隔離           |

**租戶隔離策略**：

| 客戶規模     | 隔離方式                 | 說明                              |
| :----------- | :----------------------- | :-------------------------------- |
| **小型客戶** | Row-Level Security (RLS) | 共享表格，用 `tenant_id` 欄位隔離 |
| **中型客戶** | Schema 隔離              | 每個租戶獨立 Schema               |
| **企業客戶** | Database 隔離            | 完全獨立資料庫實例                |

**Payload CMS 租戶管理**：

```typescript
// Tenants Collection 設計
{
  slug: 'tenants',
  fields: [
    { name: 'name', type: 'text' },           // 飯店名稱
    { name: 'tenant_id', type: 'text' },      // 租戶 ID
    { name: 'subdomain', type: 'text' },      // 子網域
    { name: 'line_channel_token', type: 'text', admin: { hidden: true } },
    { name: 'gemini_api_key', type: 'text', admin: { hidden: true } },
    {
      name: 'usage',
      type: 'group',
      fields: [
        { name: 'tokens_used_today', type: 'number' },
        { name: 'tokens_used_month', type: 'number' },
        { name: 'daily_limit', type: 'number' },
      ]
    },
    { name: 'status', type: 'select', options: ['active', 'suspended', 'trial'] }
  ]
}
```

**Bot 租戶路由**：

```python
# 租戶路由設計
@app.route("/webhook/<tenant_id>", methods=['POST'])
def webhook(tenant_id):
    # 從 Payload CMS 載入租戶配置
    tenant = payload_api.get_tenant(tenant_id)

    # 使用租戶專屬的 LINE Token 和 AI 設定
    config = TenantConfig(tenant)

    # 處理訊息
    return handle_message(request, config)
```

**SaaS 開發規範**：

1. **租戶意識**：

   - ✅ 所有資料操作必須帶 `tenant_id`
   - ✅ API 請求必須驗證租戶權限
   - ❌ 禁止跨租戶資料存取

2. **配置管理**：

   - 租戶配置存放於 Payload CMS
   - 敏感資訊（API Key）加密存儲
   - Bot 動態載入租戶配置

3. **用量追蹤**：
   - 每次 AI 呼叫記錄 Token 用量
   - 達到限額時降級或暫停服務
   - 月底自動重置用量

**PMS 整合規範**：

- **防腐層 (ACL)**：絕不讓 PMS 的欄位命名汙染新系統
  - `IKEY` → `bookingId`
  - `GUEST_MN` → `GuestProfile`
  - `CI_DAT` → `checkInDate`
- **MCP Server 封裝**：每個飯店的 PMS 透過獨立 MCP Server 連接
- **資料同步**：採用雙軌並行策略 (詳見第 6 章)

### 1.6 Zeabur 作業原則 ⭐

> **核心理念**：非必要不使用瀏覽器操作 Zeabur，優先使用 CLI 命令行工具。

**為什麼使用 CLI**：

| 作業方式       | 優點                         | 缺點                   |
| :------------- | :--------------------------- | :--------------------- |
| **CLI (優先)** | 可自動化、可重複執行、有記錄 | 學習曲線               |
| **瀏覽器**     | 直觀、易上手                 | 無法自動化、無操作記錄 |

**CLI 設定 (首次使用)**：

```bash
# 登入 Zeabur
npx zeabur auth login

# 設定專案 Context (只需設定一次)
source ~/ktw-projects/zeabur-switch.sh platform  # 或 bot / db
```

**常用 CLI 作業**：

| 作業         | CLI 命令                                                 |
| :----------- | :------------------------------------------------------- |
| 查看 Logs    | `npx zeabur deployment log -t runtime \| tail -100`      |
| 即時監控     | `npx zeabur deployment log -t runtime -w`                |
| 重啟服務     | `npx zeabur service restart --id=<ID> -y`                |
| 查看環境變數 | `npx zeabur variable list --id=<ID>`                     |
| 更新環境變數 | `npx zeabur variable update --id=<ID> -k "KEY=value" -y` |
| 查看資源使用 | `npx zeabur service metric CPU --id=<ID> -H 2`           |

**服務 ID 對照**：

| 服務名稱          | Service ID                 |
| :---------------- | :------------------------- |
| ktw-saas-platform | `69509aa57ab28c100bbc0aa7` |
| ktw-saas-bot      | `69509ae97ab28c100bbc0abc` |
| postgresql        | `6950c2a2479879e7ecdb006c` |

**何時可使用瀏覽器**：

- ✅ 首次設定 OAuth Callback URL（需要 UI 操作）
- ✅ 首次綁定自訂網域 DNS
- ✅ 查看完整 Deployment 歷史
- ❌ 查看 Logs（使用 CLI）
- ❌ 修改環境變數（使用 CLI）
- ❌ 重啟服務（使用 CLI）

**Workflow 文件**：

詳細命令請參考 `.agent/workflows/zeabur-logs.md` 和 `.agent/workflows/zeabur-cli-reference.md`

**錯誤修復記錄規範** ⭐：

> [!IMPORTANT]
> 任何 Zeabur 相關的錯誤與修復，**必須詳細記錄**。

**記錄位置**：`ktw-saas-platform/CHANGELOG.md` 或相關專案的 CHANGELOG

**必要記錄項目**：

| 項目           | 說明              | 範例                                            |
| :------------- | :---------------- | :---------------------------------------------- |
| **錯誤現象**   | 發生了什麼問題    | `missing secret key` 錯誤導致 404               |
| **發現時間**   | 何時發現          | 2025-12-29 21:20                                |
| **根本原因**   | 為什麼會發生      | `PAYLOAD_SECRET` 與 `DATABASE_URL` 串接在同一行 |
| **影響範圍**   | 影響了什麼功能    | 所有頁面無法載入，OAuth 登入失敗                |
| **修復方式**   | 做了什麼修復      | 在環境變數編輯器中分行並重啟                    |
| **驗證結果**   | 修復後的確認      | `✓ Ready in 539ms`，頁面正常載入                |
| **使用的命令** | 用了哪些 CLI 命令 | `npx zeabur variable update ...`                |

**記錄格式範例**：

```markdown
### 🐛 [2025-12-29] PAYLOAD_SECRET 環境變數格式錯誤

**錯誤現象**：
啟動時出現 `Error: missing secret key. A secret key is needed to secure Payload.`

**根本原因**：
Zeabur 環境變數編輯時，`DATABASE_URL` 和 `PAYLOAD_SECRET` 被串接在同一行：
`DATABASE_URL=${POSTGRES_CONNECTION_STRING}PAYLOAD_SECRET=xxx`

**修復方式**：

1. 使用 CLI 確認問題：`npx zeabur variable list --id=69509aa57ab28c100bbc0aa7`
2. 在瀏覽器中編輯原始環境變數，在兩個變數間加入換行
3. 重啟服務：`npx zeabur service restart --id=69509aa57ab28c100bbc0aa7 -y`

**驗證結果**：
```

✓ Ready in 539ms
[Zeabur] Started container ktw-saas-platform

```

```

### 1.7 輸出規範

- **測試驗證**：輸出前自己必須模擬真實測試，反覆確認。
- **語言絕對要求**：**全程繁體中文**。這包含：
  1.  **內部思考 (Thought Phase / 思考階段)**：AI 在動手前的邏輯推演。
  2.  **工具參數 (Tool Arguments / 工具參數)**：如 `task_boundary` 的 `TaskName`、`TaskStatus` 等。
  3.  **所有文檔 (Artifacts / 產出文檔)**：`Implementation Plan` (實作計畫), `Task List` (任務清單), `Walkthrough` (成果演示)。
  - **補充規則**：若遇到無法避免的英文（如 API 名稱 `generate_content`），須以 `英文 (繁體中文註解)` 格式呈現。
- **固定格式**：`Implementation Plan`, `Task List`

---

## 2. 文檔持續迭代細則 (Documentation SOP)

> ⚠️ **執行原則**：程式碼與文檔必須同步。任何功能變更若未反映在文檔中，則視為「未完成」。

### 2.1 執行時機

**一般文檔更新** (README, API 規格, Guide):

- **每次任務結束前** (在交付成果之前)
- 當修改了核心邏輯、API 介面或環境變數時

**CHANGELOG 更新** (版本記錄):

- 發布新版本時 (詳見 2.2 第 5 項)
- 重大功能變更、Bug 修復、破壞性變更時

### 2.2 自動檢查清單 (Auto-Checklist)

AI 代理在結束任務前，必須執行以下檢查：

1.  **目錄級 README**

    - 修改 `LINEBOT/` 程式碼 ➜ 更新 `LINEBOT/README.md`
    - 修改 `pms-api/` 程式碼 ➜ 更新 `pms-api/README.md`
    - **重點**：更新新功能說明、參數變更、依賴項。

2.  **API 規格文檔**

    - 變更 API 介面/格式 ➜ 更新 `pms-api/PMS-DATABASE-REFERENCE.md`。
    - 確保 `PMS-DATABASE-REFERENCE.md` 為資料庫結構的 **單一真實來源 (SSOT)**。

3.  **環境變數**

    - 新增 `.env` 變數 ➜ 同步更新 `.env.example`。

4.  **連結完整性**

    - 移動/重命名文件 ➜ 全域搜尋並修復所有斷裂的 Markdown 連結。

5.  **待辦功能記錄 (TODO.md)** ⭐

    - **適用時機**：遇到**工程較大**或**牽連較廣**的功能需求，無法立即完成時
    - **強制規定**：必須記錄到**專案根目錄** `TODO.md`
    - **記錄內容**（使用表格格式）：

    | 項目         | 說明                                             |
    | :----------- | :----------------------------------------------- |
    | **為何安排** | 觸發此待辦的事件或討論（如日期、發現問題的情境） |
    | **安排原因** | 為什麼需要做這件事，解決什麼問題                 |
    | **影響範圍** | 涉及的檔案/模組/系統                             |
    | **提出日期** | YYYY-MM-DD                                       |

    - **目的**：避免遺忘、方便後續追蹤與規劃

    **TODO 格式範例**：

    ```markdown
    ### 1. 配置 SMTP 郵件服務

    | 項目         | 說明                                                           |
    | :----------- | :------------------------------------------------------------- |
    | **為何安排** | 2025-12-23 用戶測試忘記密碼功能，發現郵件無法發送              |
    | **安排原因** | Payload 未配置郵件服務，重置密碼信只輸出到 Console，用戶收不到 |
    | **影響範圍** | 用戶忘記密碼時無法自助重置，需手動操作資料庫                   |
    | **提出日期** | 2025-12-23                                                     |

    **子任務**：

    - [ ] 選擇 SMTP 服務商 (Gmail / SendGrid / Resend)
    - [ ] 配置 Payload email adapter
    ```

    **TODO 生命週期管理** ⭐

    > [!IMPORTANT] > **核心原則**：所有未完成事項與討論結果決定執行的項目，必須經過「詢問 → 記錄 → 完成 → 核對」的完整流程。

    **流程步驟**：

    1. **發現未完成事項時**

       - AI 必須**主動詢問**：「是否要將此項目加入 TODO？」
       - 例行場景：討論中提到的待開發功能、測試未通過的項目、時間限制無法完成的項目

    2. **加入 TODO 時**

       - 必須使用上述表格格式，詳細說明**為何安排**與**安排原因**
       - 使用 `- [ ]` 格式標記子任務待完成
       - **撰寫原則**：
         - ✅ **白話**：用一般人能懂的話描述，避免過度技術術語
         - ✅ **清楚**：說明發生了什麼、為什麼會發生、影響了什麼
         - ✅ **詳細**：列出相關檔案、程式碼位置、錯誤訊息等具體資訊
       - **特別注意**：未處理完全的 Bug 記錄時，必須詳細說明：
         - 當前狀態（做到哪裡？差什麼？）
         - 卡關原因（為什麼無法完成？）
         - 下次繼續的切入點

    3. **完成項目後**

       - AI 必須**自動核對** `docs/TODO.md`，確認該項目是否已記錄
       - 如已記錄，使用**刪除線 + 打勾**方式標記完成：
         ```markdown
         - ~~`bot.py` 目前 1821 行，太肥~~ ✅ 已完成
         ```

    4. **結案前檢查**
       - AI 必須掃描 TODO.md，確認本次完成的項目都已正確標記

6.  **Bug 處理記錄 (Bug Fix Documentation)** ⭐

    > [!IMPORTANT] > **核心原則**：所有 Bug 處理完成後，必須記錄到**相對應專案的 `README.md` 最下方**，作為歷史案例參考。

    **執行時機**：

    - ✅ Bug 成功修復後
    - ✅ 問題暫時解決但需後續追蹤時（同時加入 TODO）

    **記錄位置**：

    - 記錄到**該 Bug 所屬專案**的 `README.md` 最下方
    - 範例：
      - Oracle 遷移相關 Bug → `ktw-oracle-to-pg/README.md`
      - LINE Bot Bug → `KTW-bot/LINEBOT/README.md`
      - Admin-Web Bug → `KTW-bot/ktw-admin-web/README.md`

    **記錄格式**：

    ```markdown
    ---
    
    ## 🐛 問題處理記錄
    
    ### [YYYY-MM-DD] 問題標題
    
    **問題描述**：
    用白話說明發生了什麼事
    
    **根本原因**：
    為什麼會發生這個問題
    
    **解決方式**：
    具體做了什麼修復
    
    **相關檔案**：
    - `path/to/file.py` (L123-145)
    
    **測試結果**：
    修復後的驗證結果
    
    ---
    ```

    **注意事項**：

    - ❌ 若問題**無法完全解決**，必須同時加入 `TODO.md`
    - ❌ 禁止只寫「已修復」而不說明原因和解法
    - ✅ 使用白話描述，讓未來維護者能快速理解

7.  **版本變更記錄 (CHANGELOG)** ⭐

    - **架構原則**：採用 **Monorepo 階層式變更日誌**,根目錄提供聚合摘要,各模組維護詳細記錄。
    - **執行時機**：

      - ✅ 發布新版本時 (最重要)
      - ✅ 新增功能時
      - ✅ 修復 Bug 時
      - ✅ 破壞性變更時 (Breaking Changes)
      - ✅ 重大優化時
      - **記憶法**：如果值得寫在 Git commit message,就值得寫在 CHANGELOG

    - **更新流程**：
      1. 先更新模組 CHANGELOG (如 `LINEBOT/CHANGELOG.md`)
      2. 再更新根目錄 `CHANGELOG.md` (簡潔摘要 + 連結)
    - **遵循原則**：SSOT (單一真實來源) + DRY (不重複)
    - **架構說明**：
      ```
      /CHANGELOG.md              ← 統整所有模組 (簡潔格式)
      /LINEBOT/CHANGELOG.md      ← LINE Bot 詳細記錄
      /ktw-admin-web/CHANGELOG.md ← Admin Dashboard 詳細記錄
      /ktw-backend/CHANGELOG.md  ← Backend API 詳細記錄
      /pms-api/CHANGELOG.md      ← PMS API 詳細記錄
      ```

### 2.3 CHANGELOG 編寫規範 ⭐

#### 必要元素

每個版本記錄必須包含：

1. **版本號與日期**

   ```markdown
   ## [1.2.0] - 2025-12-17
   ```

   **版本號編排規則** (Semantic Versioning):

   - **格式**: `主版本.次版本.修訂版本` (MAJOR.MINOR.PATCH)
   - **遞增規則**: 由前一版本的**最小號版本 +1** 開始編排

   **範例**:

   ```
   v1.1.6 (前一版本)
     ↓
   v1.1.7 (Bug 修復) ← 修訂版本 +1
     ↓
   v1.2.0 (新功能)   ← 次版本 +1, 修訂版本歸零
     ↓
   v2.0.0 (破壞性變更) ← 主版本 +1, 次版本與修訂版本歸零
   ```

   **何時遞增**:

   - **主版本 (X.0.0)**: 重大架構變更、不兼容的 API 修改
   - **次版本 (1.X.0)**: 新增功能、向後兼容
   - **修訂版本 (1.1.X)**: Bug 修復、小改進

2. **變更分類**

   - ✨ 新功能 (New Features)
   - 🐛 Bug 修復 (Bug Fixes)
   - ⚡ 性能優化 (Performance)
   - 🎨 UI/UX 改進 (UI/UX)
   - 📝 文檔更新 (Documentation)
   - 🔧 配置變更 (Configuration)

3. **詳細資訊** (關鍵！)
   - **檔案路徑**: 完整的相對路徑 (如 `src/App.vue`)
   - **行號範圍**: 修改的程式碼位置 (如 L197-220)
   - **修改內容**: 具體做了什麼改動
   - **修改原因**: 為什麼要這樣改
   - **影響範圍**: 影響了哪些功能

#### 範例格式

```markdown
### ✨ 新功能：已 KEY 訂單自動匹配驗證

#### 核心邏輯 (Frontend)

**檔案**: `src/App.vue`

1. **狀態計算與排序** (L197-220)

   - 新增 `hasMismatch` 狀態檢查
   - 更新 `groupStatus` 優先順序：`mismatch > pending > checked_in > cancelled`
   - 實作排序邏輯：KEY 錯訂單置頂，已 KEY 訂單過濾隱藏

2. **API 整合** (L261-282)
   - 修改 `markAsKeyed()` 函數，處理 Backend 返回的 `mismatch` 狀態
   - 延長 API timeout 至 10 秒（因需查詢 PMS）
   - 新增錯誤處理：匹配失敗時刷新列表顯示 KEY 錯狀態

### 🐛 Bug 修復

1. **多房型床型錯誤**
   - **檔案**: `bot.py` (L1171)
   - **問題**: 所有房型共用同一個 `bed_type`
   - **修復**: 解析床型字串，為每個房型分配正確床型
   - **影響**: 修復多房型預訂時床型資訊錯誤的問題

### 📝 修改的文件

- `src/App.vue` (L197-220, L261-282) - 狀態邏輯、API 整合
- `src/style.css` (L1670-1706) - 新增 mismatch 相關樣式
```

#### 禁止事項

- ❌ 只寫「修復 Bug」而不說明具體問題
- ❌ 只寫「優化性能」而不說明優化了什麼
- ❌ 不寫檔案路徑和行號
- ❌ 不說明修改原因

### 2.4 README 編寫規範

#### 必要章節

每個模組的 `README.md` 必須包含：

1. **模組概述**

   - 一句話說明模組用途
   - 核心功能列表 (用 ✅ 標記已完成功能)

2. **技術架構**

   - 技術棧 (語言、框架、主要依賴)
   - 系統架構圖 (用 ASCII 或 Mermaid)

3. **檔案結構**

   - 目錄樹狀圖
   - 關鍵檔案說明

4. **快速開始**

   - 環境需求
   - 安裝步驟
   - 啟動指令

5. **核心功能說明**

   - 每個主要功能的詳細說明
   - 使用範例

6. **開發指南**

   - 如何修改配置
   - 如何新增功能
   - 相關文檔連結

7. **故障排除**

   - 常見問題與解決方案

8. **版本資訊**
   - 當前版本號
   - 最後更新日期
   - CHANGELOG 連結

#### 範例格式

```markdown
# 模組名稱

> 一句話說明模組用途

---

## 📋 模組概述

### 核心功能

- ✅ 功能 1
- ✅ 功能 2
- ⏸️ 功能 3 (開發中)

---

## 🏗️ 技術架構

### 技術棧

- **語言**: Python 3.9+
- **框架**: Flask

### 系統架構

\`\`\`
用戶 → API → 資料庫
\`\`\`

---

## 📁 檔案結構

\`\`\`
module/
├── README.md
├── CHANGELOG.md
└── src/
\`\`\`

---

## 🚀 快速開始

### 環境需求

- Python 3.9+

### 安裝

\`\`\`bash
pip install -r requirements.txt
\`\`\`

---

## 📝 版本資訊

- **當前版本**: v1.2.0
- **最後更新**: 2025-12-17
- **維護者**: KTW Hotel IT Team

詳細變更記錄請參閱 [CHANGELOG.md](./CHANGELOG.md)
```

#### 更新時機

- 新增功能時 → 更新「核心功能」章節
- 修改架構時 → 更新「技術架構」章節
- 新增環境變數時 → 更新「快速開始」章節
- 修改版本時 → 更新「版本資訊」章節

---

## 3. 程式修改前置標準流程 (Pre-Coding SOP)

> ⚠️ **執行原則**：在修改任何程式碼之前，必須嚴格執行以下三步驟。禁止直接寫 Code。

### 步驟 1：歷史回顧與文件查閱 (Context Retrieval)

- **強制查閱**：必須先閱讀相關的 `README.md`、`GUIDE.md` 或架構文檔。
- **確認現況**：理解現有程式碼的設計意圖，避免破壞既有邏輯 (Regression)。

### 步驟 2：前瞻後顧與最佳模式分析 (Impact Analysis)

- **最佳模式**：思考「這是業界的最佳實踐 (Best Practice) 嗎？」
- **前瞻**：這個修改對未來擴充有幫助嗎？會不會造成技術債？
- **後顧**：這個修改會不會影響舊功能？是否與現有架構衝突？

### 步驟 3：方案研擬與利弊分析 (Scenario Planning)

在動手前，必須向使用者提出 **3 種解決方案**，並進行利弊分析：

| 方案  | 說明       | 優點 (Pros) | 缺點 (Cons) | 建議程度 |
| :---: | :--------- | :---------- | :---------- | :------: |
| **A** | (方案名稱) | ...         | ...         |  ⭐⭐⭐  |
| **B** | (方案名稱) | ...         | ...         |   ⭐⭐   |
| **C** | (方案名稱) | ...         | ...         |    ⭐    |

- **討論與決策**：與使用者討論並達成共識後，才可進入實作階段 (Coding)。

---

## 4. API 設計思維 (API Design Philosophy)

在面對複雜的舊系統時，API 不應只是資料庫的 CRUD 介面，而是**業務能力的封裝**。

### 4.1 核心原則

1.  **意圖導向 (Intent-Based)**

    - ❌ **Don't**: `GET /api/guest_mn?id_cod=...` (暴露底層資料表)
    - ✅ **Do**: `GET /api/guests/search?identity=...` (封裝業務動作)
    - **理由**：解耦前後端。底層資料來源可能從 Oracle 換成 PostgreSQL，但 API 介面應保持穩定。

2.  **防腐層 (Anti-Corruption Layer, ACL)**

    - **原則**：絕不讓舊系統的髒命名 (Legacy Naming) 汙染新系統。
    - **實作**：在 API 層 (Node.js) 進行轉換。
      - `IKEY` ➜ `bookingId`
      - `GUEST_MN` ➜ `GuestProfile`
      - `CI_DAT` ➜ `checkInDate`

3.  **讀取優化 (Read-Optimized)**
    - **現況**：舊系統過度正規化，導致查詢緩慢。
    - **策略**：針對高頻查詢（如「今日入住名單」），在 API 層做聚合 (Aggregation)，一次回傳完整 JSON，減少前端往返次數。

---

## 5. 舊資料庫探索技巧 (Legacy DB Exploration)

Oracle PMS 是一個黑盒子，探索需講求方法。

### 5.1 探索 SOP

1.  **Trace > Guess (追蹤勝於猜測)**

    - 優先查看 Application Log 或開啟 SQL Trace，觀察系統在執行特定動作（如 check-in）時觸發了哪些 SQL。這比看欄位名稱猜測準確得多。

2.  **Data-Driven Inference (數據推斷)**

    - 不確定欄位用途時，使用 `SELECT DISTINCT` 查看實際存儲的值。
      ```sql
      -- 用數據說話：看 GUEST_STA 到底有哪些狀態
      SELECT DISTINCT GUEST_STA, COUNT(*) FROM GUEST_MN GROUP BY GUEST_STA;
      ```

3.  **建立映射文檔 (Mapping Document)**

    - 持續維護 `pms-api/PMS-DATABASE-REFERENCE.md`，這是團隊最重要的資產。

4.  **協作分析與討論 (Collaborative Analysis)** ⭐
    - **原則**：查到任何不明確的欄位或數據時，**務必提交給使用者分析**。
    - **理由**：使用者熟悉舊系統的前端邏輯（Frontend Logic），能從欄位值中發現工程師看不出的業務線索（例如：某個 Flag 代表的特定業務流程）。
    - **行動**：在 Task List 或討論中列出 `[欄位名]: [範例值]`，邀請使用者共同判讀。

### 5.2 PMS 欄位表格維護規範 ⭐

> [!IMPORTANT] > **核心原則**：`pms-api/PMS-DATABASE-REFERENCE.md` 是資料庫結構的 **單一真實來源 (SSOT)**。
>
> 此文件的準確性直接影響未來自建資料庫的正確性，必須嚴格維護。

**維護流程**：

1.  **發現新欄位或代碼**

    - 透過 SQL 查詢 (`SELECT DISTINCT`) 探索欄位的可能值
    - 記錄原始欄位名、實際數據、推測用途

2.  **交叉比對驗證**

    - 與 PMS 系統的實際行為進行比對
    - 確保欄位描述與觀察到的數據一致

3.  **提交使用者確認** ⭐

    - **強制規定**：任何新發現的欄位或代碼說明，必須先與使用者確認後才能寫入文檔
    - **理由**：使用者了解業務邏輯，能識別 AI 或工程師可能誤解的專業術語
    - **格式**：提供表格形式的對照說明，方便使用者快速審閱

4.  **更新文檔並記錄**
    - 確認後更新 `PMS-DATABASE-REFERENCE.md`
    - 在 CHANGELOG 中記錄欄位定義的變更

**範例：瑕疵房 vs 維修房**

| 類型   | 判斷條件                        | 說明           | 可售？ |
| :----- | :------------------------------ | :------------- | :----: |
| 瑕疵房 | `OOS_STA = Y` 且 `ROOM_STA = V` | 有小問題但可賣 |   ✅   |
| 維修房 | `ROOM_STA = R`                  | 正在維修       |   ❌   |

> **此定義經 2025-12-21 使用者確認**

---

## 6. 資料庫遷移策略 (Migration Strategy)

### 6.1 戰略：絞殺榕模式 (Strangler Fig Pattern)

絕對禁止採用 Big Bang (一次性切換) 模式。應採用**雙軌並行**策略，逐步替換。

### 6.2 執行階段 (Phases)

#### Phase 1: 旁路模式 (Sidecar) - **目前階段**

- **狀態**：Oracle (Master) ➜ PostgreSQL (Read-Only Slave/Cache)
- **機制**：單向同步。寫一個排程器 (Scheduler) 每 5 分鐘從 Oracle 撈取異動資料，寫入 PostgreSQL。
- **用途**：LINE Bot、Admin Dashboard 讀取 PostgreSQL，速度快且不影響舊系統效能。寫入操作仍回到 Oracle。

#### Phase 2: 增量寫入 (Incremental Write)

- **狀態**：新功能資料直接寫入 PostgreSQL。
- **案例**：LINE 加好友紀錄、Wifi 授權紀錄、AI 對話日誌。這些是舊系統沒有的功能，直接在新架構落地。

#### Phase 3: 影子寫入 (Shadow Write)

- **狀態**：嘗試接管寫入。
- **機制**：當用戶在 Bot 訂房時，API 同時寫入 PostgreSQL 和 Oracle（透過原廠介面或模擬操作）。比對兩邊結果是否一致，確保邏輯正確。

#### Phase 4: 切換 (Cutover)

- **狀態**：PostgreSQL 升級為主庫 (Master)。
- **條件**：Oracle 降級為僅供查詢的歷史存檔庫 (Archive)。

---

## 8. Oracle 欄位核對規則 (Field Verification) ⭐

> **適用範圍**：`ktw-oracle-to-pg` 專案中的欄位探索與 L3 建立流程。

### 8.1 我必須提供的格式

| 欄位    | 值         | 備註                     |
| :------ | :--------- | :----------------------- |
| XXX_STA | Y          | 狀態？ (Y=啟用? N=停用?) |
| XXX_DAT | 2025-12-31 | 日期？                   |

> **注意**：推測前必須遵守 1.1 節「研究導向」原則。

### 8.2 備註規則

- **有對照表的欄位**：必須列出所有可能值 + 推測
  - 例：`狀態？ (Y=啟用? N=停用? R=維修?)`
- **問號?**：代表未確認
- **無問號**：代表已確認

### 8.3 核對流程

| 步驟 | 動作                                       |
| :--: | :----------------------------------------- |
|  1   | 我提供：欄位 + 值 + 備註？                 |
|  2   | 用戶回貼確認版（去掉 ? 或修改內容）        |
|  3   | 沒 ? 的就是確定                            |
|  4   | 我寫入 `oracle_metadata.column_comments`   |
|  5   | 建立/更新 L3 View + COMMENT ON COLUMN      |
|  6   | 關聯 L1、L2 作業（同步、table_catalog 等） |

### 8.4 確認範例

**我提供：**

```
BED_STA | S | 床位狀態？ (N=標準? M=併床? S=分床?)
```

**用戶回貼（確認，去掉?）：**

```
BED_STA | S | 床位狀態 (N=標準 M=併床 S=分床)
```

**或用戶回貼（修正內容）：**

```
BED_STA | S | 床位狀態 (N=一般床 M=雙人床 S=單人床)
```

**我執行：**

1. 寫入 `oracle_metadata.column_comments`
2. 更新 L3 View COMMENT
3. 加入同步清單（如需要）

---

## 9. 結論 (Summary)

- **API 層**：做翻譯官，不要做傳聲筒。把 `IKEY` 翻譯成 `bookingId` 再給前端。
- **DB 層**：先求共存，再求取代。從「讀取優化」開始，建立信心後再處理「寫入」。

---

_文件最後更新：2025-12-28_
