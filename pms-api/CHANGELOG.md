# PMS REST API - Changelog

> PMS 資料庫 API 服務的詳細變更記錄

---

## [1.9.3] - 2025-12-23

### ✨ 漸進式暫存支援

**修改檔案**：`routes/bookings.js`

1. **incomplete 狀態跳過驗證** (L1114-1123)
   - 當 `status === 'incomplete'` 時，跳過房型/姓名/電話/抵達時間的必填驗證
   - 用於 Bot 意圖確認時的初始暫存

2. **LINE ID 核對避免重複** (L1184-1197)
   - 新增用 `line_user_id` 核對同日 incomplete/pending 訂單
   - 找到就更新，沒找到才新增，避免重複訂單

---

## [1.9.2] - 2025-12-21

### ✨ 新功能：遠端 LOG 讀取 API

> **背景**：為方便遠端查看伺服器日誌，新增 LOG 讀取 API 端點。

#### 新增檔案
- **`routes/logs.js`** - LOG 讀取路由

#### 新增端點

| 端點 | 說明 |
|:-----|:-----|
| `GET /api/logs` | 列出所有可用的 LOG 檔案 |
| `GET /api/logs/today` | 取得今日 LOG 內容 |
| `GET /api/logs/:date` | 取得指定日期 LOG (YYYY-MM-DD) |

#### 查詢參數

| 參數 | 說明 | 範例 |
|:-----|:-----|:-----|
| `tail=N` | 只取最後 N 行 | `?tail=50` |
| `level=X` | 過濾等級 (ERROR, ORACLE, REQUEST...) | `?level=ERROR` |
| `search=X` | 搜尋關鍵字 | `?search=booking` |

#### 修改檔案
- **`server.js`** (L20, L26, L32) - 註冊 logs 路由
- **`server.js`** (L47) - API 版本號更新為 1.8

**使用範例**:
```bash
# 取得今日最後 20 行 LOG
curl "http://192.168.8.3:3000/api/logs/today?tail=20"

# 只看錯誤
curl "http://192.168.8.3:3000/api/logs/today?level=ERROR"
```

---

## [1.9.1] - 2025-12-21

### 🐛 Bug 修復：OTA 訂單查詢順序調整

> **問題背景**：客人提供 `1671721966`，無法匹配到 `RMAG1671721966`。

#### 查詢策略重構
- **檔案**: `routes/bookings.js` (L846-930)
- **修正**: 調整三層查詢順序，優先 OTA 模糊匹配

| 優先順序 | 原策略 | 新策略 |
|:--------:|:-------|:-------|
| 1 | IKEY 精確匹配 | ⭐ **OTA 模糊匹配** |
| 2 | OTA 精確匹配 | IKEY 精確匹配 |
| 3 | 全模糊匹配 | IKEY 模糊匹配 |

### ✨ 新功能：API Logger (伺服器端日誌)

- **新增檔案**: `helpers/apiLogger.js`
- **功能**:
  - 記錄所有 API 請求與回應
  - 記錄 Oracle 查詢耗時
  - 記錄 Oracle 錯誤詳情
  - **3 天自動清理**
- **日誌位置**: `logs/pms_api_YYYY-MM-DD.log`

**日誌格式範例**:
```
10:24:58 | REQUEST | GET /bookings/1671721966
10:24:58 | DEBUG | 查詢訂單: 1671721966 (優先 OTA 模糊匹配)
10:24:58 | ORACLE | FIND_ORDER | elapsed=5ms | rows=1
10:24:58 | RESPONSE | GET /bookings/1671721966 | status=200 | elapsed=60ms
10:24:58 | INFO | 訂單查詢成功: 00703101 (OTA: RMAG1671721966)
```

---

## [1.9.0] - 2025-12-21

### ✨ 新功能：動態日期入住查詢 (Dynamic Check-in Query)

1. **新增 `/checkin-by-date` 端點**
   - **檔案**: `routes/bookings.js` (L306-366)
   - **內容**: 實作支援 `date` 與 `offset` 參數的查詢邏輯。
   - **理由**: 原有端點僅支援今日/明日，無法滿足週報與月報的歷史或遠期查詢需求。
   - **影響**: LINE Bot 能準確查詢任意日期的詳細入住名單（含房號）。

### 🔧 部署管理標準化 (Operation Standards)

1. **NSSM 服務管理標準化**
   - **路徑**: `C:\ktw-bot\pms-api\nssm.exe`
   - **內容**: 確認服務名稱為 `KTW_PMS_API`。
   - **異動**: 將重啟指令與工作流路徑更新為絕對路徑，確保 SSH 執行穩定性。

---

## [1.8.0] - 2025-12-19

### 🔧 服務穩定性與資料庫維修

#### 1. Oracle 資料庫磁碟/實例故障修復 (Server Maintenance)
**情境**: 遠端伺服器 (192.168.8.3) 返回 `ORA-01034: ORACLE not available` 錯誤。
- **變更**：透過 SSH 遠端管理權限執行資料庫維修。
- **操作**：
    1. 執行 `net stop OracleServiceGDWUUKT` 停止掛起的服務。
    2. 執行 `net start OracleServiceGDWUUKT` 重新初始化資料庫實例。
- **結果**：PMS API 恢復正常連線，解決了系統全面崩潰的 critical blocker。

#### 2. 檔案同步路徑對齊
- **變更**：配合 Backend API 修復路徑中的 `data/` 前綴。

---

## [1.7.3] - 2025-12-19

### ✨ 新增功能：OTA 訂單號碼欄位

**檔案**: `routes/bookings.js` (L747-749, L770-772, L794-796, L860)

**新增欄位**: `ota_booking_id`

**背景**：
- LINE Bot 需要顯示外部訂單編號（如 Agoda 的 RMAG1667798817）而非內部 ID（00701101）
- 先前 API 雖然能查詢 OTA 訂單號，但未在回傳 JSON 中包含此欄位

**修改內容**：

1. **SQL 查詢補齊** (3 個查詢策略)
   ```sql
   -- 在所有查詢中加入
   TRIM(om.RVRESERVE_NOS) as ota_booking_id
   ```
   - 策略 1：IKEY 精確匹配 (L749)
   - 策略 2：RVRESERVE_NOS 精確匹配 (L772)
   - 策略 3：模糊匹配 (L796)

2. **JSON 回傳結構補齊** (L860)
   ```javascript
   {
     booking_id: "00701101",        // 內部 ID
     ota_booking_id: "RMAG1667798817",  // OTA 外部編號（新增）
     guest_name: "...",
     // ... 其他欄位
   }
   ```

**測試結果**：
```bash
curl "http://192.168.8.3:3000/api/bookings/1667798817"
# 返回包含 "ota_booking_id": "RMAG1667798817" ✅
```

**影響範圍**：
- LINE Bot 現在能正確顯示 OTA 訂單編號
- 提升用戶體驗（客人更容易核對訂單）

---

## [1.7.2] - 2025-12-19

### 🔧 Bug 修復：OTA 訂單號查詢功能

**檔案**: `routes/bookings.js` (L731-807)

**問題**: 用戶使用 Agoda/Booking 訂單號查詢時查不到訂單

**根本原因**: OTA 訂單號存儲在 `RVRESERVE_NOS` 欄位，但查詢邏輯只查 `IKEY` 欄位

**解決方案**: 實作三層查詢策略

1. **第一層：IKEY 精確匹配** (L732-750)
   - 查詢 PMS 內部訂單號
   - 例如：`00701101`

2. **第二層：RVRESERVE_NOS 精確匹配** (L753-773)
   - 查詢 OTA 訂單號（新增）
   - 支援 `RMAG1667798817` (Agoda)
   - 支援 `RMPGP250298503` (官網)

3. **第三層：模糊匹配** (L776-797)
   - 同時在 IKEY 和 RVRESERVE_NOS 中模糊匹配
   - 支援只用數字部分查詢（例如：`1667798817` → `RMAG1667798817`）

**測試結果**:
- ✅ `1667798817` → 成功找到 `RMAG1667798817`
- ✅ `250298503` → 成功找到 `RMPGP250298503`  
- ✅ `RMAG1677888347` → 精確匹配成功

**影響範圍**: 
- 修復 Bot 訂單查詢失敗問題
- 大幅提升用戶體驗（支援多種訂單號格式）

---

## [1.7.1] - 2025-12-18

### ⚡ API 簡化與優化 (Optimization)

#### 1. 移除冗餘查詢參數
**檔案**: `routes/bookings.js` (L18-60)

- **修改內容**：從 `/api/bookings/search` 端點移除 `id` 查詢參數。
- **原因**：為了減少與既存單一訂單查詢端點 `GET /api/bookings/:booking_id` 的功能重疊與混淆。
- **影響**：`/search` 端點現在僅接收 `name` 與 `phone` 參數，符合專案 Scenario B 的架構規劃。

---

## [1.7.0] - 2025-12-17

### ✨ 新功能：訂單狀態回寫

#### 1. Mismatch 狀態標記
**檔案**: `routes/bookings.js`

- **新增端點**: `PATCH /api/bookings/same-day/:order_id/mismatch` (L586-650)
- **功能**: 將 `same_day_bookings.json` 中的訂單狀態更新為 `mismatch`
- **用途**: 當 Admin Web 點擊「已 KEY」但自動比對失敗時呼叫
- **實作**:
  ```javascript
  // 讀取訂單 -> 找到對應 ID -> 更新 status = 'mismatch' -> 寫回檔案
  targetBooking.status = 'mismatch';
  await fs.promises.writeFile(itemPath, JSON.stringify(bookings, null, 2));
  ```

#### 2. Check-in 邏輯增強
**檔案**: `routes/bookings.js` (L542-610)

- **修改**: `checkin` 端點現在會先執行 PMS 電話比對
- **邏輯**: 
  - Admin Web 觸發 -> Backend API 驗證 -> 比對成功 -> 呼叫 PMS `/checkin`
  - 比對失敗 -> Backend API 呼叫 PMS `/mismatch`

### 📝 修改的文件
- `routes/bookings.js` (L542-610, L586-650) - 新增與修改 Check-in/Mismatch 邏輯

---

## [1.6.0] - 2025-12-11

### ✨ 新功能：Windows 服務支援

1. **服務管理腳本**
   - **檔案**: `manage-service.bat`
   - **功能**: 整合 Start/Stop/Restart/Status 功能的批次檔
   - **位置**: 專案根目錄

2. **安裝/移除腳本**
   - **檔案**: `install_service.js`, `uninstall_service.js`
   - **功能**: 使用 `node-windows` 將 API 註冊為 Windows 本地服務
   - **設定**: 自動處理連線重試與錯誤重啟

### 🐛 Bug 修復

1. **Oracle 連線超時 (ORA-12170)**
   - **檔案**: `.env`, `listener.ora` (Server Side)
   - **問題**: `ORACLE_HOME` 環境變數衝突與 SID 配置錯誤
   - **修復**: 修正環境變數並重啟監聽器

2. **防火牆阻擋**
   - **設定**: Windows Firewall
   - **修復**: 開放 Inbound Port 3000

---

## [1.0.0] - 2025-12-10

### 初始版本
- 建立 Oracle DB 連線池
- 訂單查詢 API (Search by name/phone)
- 房況查詢 API (Room availability)
- 訂單詳情 API
