...
- ✅ 語音訊息辨識
- ✅ 圖片訂單識別
- ✅ VIP 客戶管理 (Modular FSM)
- ✅ 營運報表精準化與預測
- ✅ 對話狀態持久化
- ✅ **AI 意圖判斷查詢** (v1.9.0 NEW)
- ✅ **昨日/特定日期房況查詢** (v1.9.0 NEW)
...
### 3. 當日預訂
- **架構**: AI 驅動 (System + Function Calling)
- **解析**: 支援自然語言多房型解析 (如「2間雙人1間三人」)
- **流程**: 意圖識別 → 庫存查詢 → 房型確認 → 建立訂單
...
### 6. VIP 內部查詢系統 (v1.9.0)
- **AI 意圖判斷**: 使用 Gemini AI 理解口語化查詢
- **今日房況**: 「今天住幾間」、「住房率」
- **昨日/特定日期**: 「昨天住幾間」、「12/25有幾間預訂」
- **週/月統計**: 「本週預測」、「本月入住狀況」
- **房型顯示智慧化**:
  - 過去/今日：顯示實際房型（從房號查詢）
  - 未來日期：顯示訂單房型（計價房種）
