...
> **背景**：Gemini 語音轉文字有「幻覺」問題，當音訊不清楚時會自行生成「看起來像 Bot 回覆」的內容。

#### 修改內容

- **檔案**: `bot.py` (L772-820)
- **變更**: 語音轉文字從 Gemini 改為 OpenAI Whisper API
- **優點**:
  - 準確度更高
  - 無幻覺問題
  - 費用極低（$0.006/分鐘）
...
> **背景**：VIP 用戶問「現在的天氣」時，Bot 回覆佔位符內容而非真正查詢天氣。

#### 問題原因

- VIP 的一般問答走 `_free_chat()` fallback
- `_free_chat()` 沒有 function calling，無法調用 `get_weather_forecast`

#### 修復方式

- **檔案**: `handlers/vip_service_handler.py` (L81-123)
- **變更**: 移除 `_free_chat()` fallback，讓一般問答交回主流程處理
- **效果**: VIP 的天氣、景點等問題現在會正確調用 function calling
...
- **新增**: 官網訂單取消/改期政策
  - 一般日期：可協助改期或取消
  - 國定連續假日：取消視同如期入住，已付訂金恕不退還

### 📝 修改的文件

- `bot.py` (L772-820) - 語音辨識改用 Whisper
- `handlers/vip_service_handler.py` (L81-123) - VIP 一般問答交回主流程
- `data/knowledge_base.json` - 新增官網取消政策
...
> **背景**：分析「阿化」客人對話後發現，訂單編號 `250277285` 被誤判為電話或抵達時間，導致對話卡住。根本原因為 Handler 使用硬解析正則，無視 AI 判斷結果。

#### 核心改進：AI + Helper 雙層驗證

```
客人訊息 → AI 判斷意圖 → Handler 路由 → Helper 驗證格式 → 正確結果
```

#### 1. 強化電話驗證邏輯

**檔案**: `helpers/intent_detector.py` (L185-246)

- **變更**: `extract_phone_number()` 新增 `strict` 參數
  - `strict=True`：只接受台灣電話格式（09 開頭手機、0X 市話）
  - `strict=False`：向後兼容舊邏輯
- **新增**: `is_phone_number()` 輔助方法
- **效果**: 訂單號 `250277285` 不再被誤判為電話

#### 2. 新增訂單意圖偵測

**檔案**: `helpers/intent_detector.py` (L66-120)

- **新增**: `is_new_order_query(message, current_order_id)`
  - 偵測客人是否在對話中提到「另一筆」訂單
  - 支援 OTA 前綴清理比對（RMPGP、RMAG 等）
- **新增**: `is_possible_order_number(message)`
  - 簡單邏輯：不是 0 開頭的 5 位以上數字 = 可能是訂單
- **效果**: 觸發訂單切換確認，而非強制處理

#### 3. 新增特殊需求偵測

**檔案**: `helpers/intent_detector.py` (L337-413)

- **新增**: `is_special_request()`, `extract_special_request()`
- **涵蓋**: 40+ 關鍵字（嬰兒床、禁菸、鄰近、停車、大床等）
- **效果**: 客人說「兩筆訂單能安排鄰近嗎？」不再被忽略

#### 4. 新增時間格式驗證

**檔案**: `helpers/order_helper.py` (L192-355)

- **新增**: 4 個時間相關方法
  - `convert_chinese_numerals()`: 三 →3、五 →5
  - `is_valid_time_format()`: 判斷是否為有效時間
  - `validate_arrival_time()`: 驗證+標準化
  - `is_vague_time()`: 判斷是否模糊需追問
- **效果**: 訂單號 `250277285` 不再被誤判為抵達時間

#### 5. 重構 OrderQueryHandler

**檔案**: `handlers/order_query_handler.py` (L1-16, L331-420)

- **更新 import**: 引入 `IntentDetector`, `validate_arrival_time`, `is_vague_time`
- **`_handle_order_confirmation()`**: 加入特殊需求偵測
- **`_handle_phone_collection()`**: 加入新訂單偵測 + 嚴格電話驗證
- **`_handle_arrival_collection()`**: 加入新訂單偵測 + 時間格式驗證

#### 6. 預備性升級：SameDayBookingHandler

**檔案**: `handlers/same_day_booking.py` (L1-14)

- **新增 import**: 引入 `IntentDetector`, `validate_arrival_time`, `is_vague_time`
- **用途**: 預備未來按需升級
...
- `helpers/intent_detector.py` - 新增 6 個方法
- `helpers/order_helper.py` - 新增 4 個時間相關方法
- `handlers/order_query_handler.py` - 3 個方法重構
- `handlers/same_day_booking.py` - import 更新

---

## [1.9.7] - 2025-12-21

### ✨ AI 模型升級：Gemini 3 Pro

> **背景**：Gemini 3 Flash 傾向「快速回答」，遇到「訂單號+問題」時會一次回答全部，導致客人得到答案就不再回覆，無法收集資料。

#### 1. 主模型升級

**檔案**: `bot.py` (L452-459)

- **變更**: 主模型從 `gemini-3-flash-preview` 升級至 `gemini-3-pro-preview`
- **理由**: Pro 版適合「需要跨模態進階推理的複雜工作」，更願意遵循多步驟流程
- **Flash 保留**: 一般對話仍使用 Flash 以維持回應速度

#### 2. 溫度參數調整

**檔案**: `bot.py` (L438-445)

- **變更**: 溫度從 0.2/0.5 統一改為 1.0
- **依據**: [官方文檔](https://ai.google.dev/gemini-api/docs/gemini-3)建議維持 1.0，低於此值可能導致迴圈問題

#### 3. 新增問題緩存策略 (Question Buffering)

**檔案**: `bot.py` (L130-160)

- **新指令**: 當客人「訂單號 + 問題」一起發送時
  1. 記住問題但不立即回答
  2. 先完成訂單確認 + 資料收集
  3. 最後才統一回答問題
- **目的**: 避免客人得到答案就離開，確保能收集到 LINE 姓名與聯絡資訊

### 📝 修改的文件

- `bot.py` (L130-160, L438-459) - 系統指令與模型配置
...
- **修改**: `query_month_forecast()` 現在顯示整個月（1 號到月底）
- **分區顯示**: 已過日期、今日、未來日期
...
- **房型合併統計** (L193-208)：相同房型自動合併計數（4 間標準雙人房顯示為「標準雙人房 x4」而非重複列舉）。
